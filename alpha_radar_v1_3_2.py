# -*- coding: utf-8 -*-
"""
Alpha Radar v1.3.2  (FMZ 单文件策略)
================================================================
研究型价格结构观测系统。无任何实盘发单逻辑。
蓝图: alpha_radar_spec v1.3.2 / 参考: gamma_spatial_observer_v6.py
...
v1.3.1 外科式修补:
  - Fix-1 Chart Flag 补齐 anchor_shift 分支
  - Fix-2 EXPIRED 状态下 anchor_health_score = None
  - Fix-3 ppe_percentile 先算后 append (消除自包含)
  - Fix-4 Display 注释-实现脱节清理
  - Fix-5 CVD label 零偏离返回 neutral
  - Fix-6 anchor_shift_frac 与 anchor_ppe_reset_frac 两档分层

v1.3.2 时效性防御 + 展示语义收口 + 健康度观测出口:
  E-1 BarAssembler 新增 poll_with_drain() —— 受控 catch-up drain
      三道护栏: max_drain_rounds / max_drain_wall_time_ms / 零增量退出
      解决: 主循环慢 + REST pull 模型导致的 backlog（旧成交堆积）
      不解: 真实 trade_id gap（REST 先天无法回补，仍依赖 CVD 降级保护）
  E-2 Display 缺值语义三态收口
      正常值 / 未就绪（COLD_START/EXPIRED，有业务语义）/ - （确实缺值）
  E-3 LogProfit 改出口: 由"归一化偏差"改为"锚健康度"
      score=None 时不调用（不伪造 0，不复用上次值）
      偏离信息保留在状态栏、主图 σ 副轴、summary_log
  E-4 raw_values 新增 2 个采集度量字段（最少而够用）:
      poll_wall_time_ms / catchup_rounds_used
  E-5 backlog_hint 仅在 Display 状态栏表达，不入快照、不入 FSM
================================================================
"""
...
class Display:
    ...
    def on_bar(self, snapshot, anchor_ctx, bar_asm, dev_state, labels,
        ...
        # v1.3.2: LogProfit 改出口，传 labels 消费 anchor_health_score
        self._update_logprofit(labels)
    ...
    def _update_logprofit(self, labels):
        """
        v1.3.2 重定向:
          旧行为: LogProfit 输出 normalized_deviation（价格偏离 σ）
          新行为: LogProfit 输出 anchor_health_score（锚健康度，0~100）

        设计决策:
          score=None (COLD_START / EXPIRED) → 跳过本 tick，不调用 LogProfit
          理由: LogProfit 图上出现断点是正确的视觉语义——"不可计算" ≠ "为零"，
                不伪造 0，不复用上次值（会误导时间序列读数）。

          直接输出 0~100 原始值，不归一化为 [0, 1]:
          理由: anchor_health_score 的分档阈值 80/60/40/20 已深入规范与 UI，
                归一化后用户读图还要做心算，丢失直觉。

          更新节奏: 沿用现有 logprofit_interval_sec (10s)，不新增独立参数。

          偏离信息的职责分离 (v1.3.2):
            - 状态栏"因子证据"表的"归一化偏差"行        → 瞬时数值
            - 主图右轴"偏差(σ)"曲线                     → 时间序列
            - summary_log 的 "偏差={nd}σ" 字段          → 日志回看
            - LogProfit (本出口) 不再承担偏离观测职责
        """
        now_sec = int(time.time())
    ...
    def _update_status(self, raw_row, labels, system_state,
        ...
        # v1.3.2: 采集时效性表 (2 行，轻量)
        # 该表消费 BarAssembler 的 drain 度量，回答"观察者看到的是实时市场还是滞后市场"
        table_collection = self._build_collection_table(raw_row, bar_asm)

        log_status(summary, tables=[table_anchor, table_factors,
                                     table_state, table_events,
                                     table_health, table_collection])
    ...
    def _build_collection_table(self, raw_row, bar_asm):
        """
        v1.3.2 新增《采集时效性》状态栏表。

        回答的核心问题: 观察者看到的是实时市场，还是滞后的旧市场？

        两行设计（轻量不膨胀）:
          1) 采集节奏 — drain 轮数 / 耗时 ms，快速判断"主循环追得上吗"
          2) Backlog  — 是否存在未消化积压 + CVD 降级状态

        关键语义区分（v1.3.2 对用户的承诺）:
          backlog  ≠  trade_id gap
          ────────────────────────────────
          backlog :
            系统正在处理旧成交，但 trade_id 仍然连续。
            Bar 结构仍然对，只是"延迟"。catch-up drain 试图消灭它。
          trade_id gap:
            REST 单轮 1000 条装不下中间成交被跳过，
            CVD 方向被动降级 (cvd_degraded=True)，这是数据完整性破坏。
            Bar 的 Volume 记账在持续 gap 下会失真（已知局限）。

        展示形式:
          - 采集节奏: 显示 catchup_rounds_used × poll_wall_time_ms
                      + 如触发 drain 超时额外标注 [超时]
          - Backlog : 显示 "正常"/"积压" + CVD 降级标注
        """
        raw_safe = raw_row or {}

        # ── 行 1: 采集节奏 ─────────────────────────────────────
        rounds_used  = raw_safe.get("catchup_rounds_used")
        wall_time_ms = raw_safe.get("poll_wall_time_ms")

        if rounds_used is None or wall_time_ms is None:
            # 普通缺值（raw 层没收到采集度量），显示 "-"
            tempo_cell   = "-"
            tempo_detail = "-"
        else:
            # 正常可计算
            if rounds_used <= 1:
                ...
                tempo_cell = "{} 轮 drain".format(rounds_used)
            tempo_detail = "{} ms".format(wall_time_ms)

        # 采集层原生的超时标记（通过 bar_asm 直读，不经快照）
        try:
            metrics = bar_asm.get_last_cycle_metrics()
            if metrics.get("hit_wall_time"):
                tempo_detail = tempo_detail + " [超时]"
        except Exception:
            pass

        row_tempo = ["采集节奏", tempo_cell, tempo_detail]

        # ── 行 2: Backlog 状态 ─────────────────────────────────
        # Backlog 判定由 BarAssembler 层完成（靠近采集层），Display 只消费。
        try:
        ...
        backlog_cell = "积压" if is_backlogged else "正常"
        if is_degraded:
            backlog_detail = "CVD 降级 (trade_id gap)"
        elif is_backlogged:
            backlog_detail = "旧成交未消化完"
        else:
            backlog_detail = "连续且已追上"

        row_backlog = ["Backlog", backlog_cell, backlog_detail]

        return {
            "type":  "table",
            "title": "采集时效性",
            "cols":  ["指标", "当前值", "说明"],
            "rows":  [row_tempo, row_backlog],
        }
...
def run():
    ...
    主策略循环。每个 tick (loop_sleep_sec) 执行:
      ...
      2. poll_with_drain()    — Binance aggTrades → 新等币量柱（含 catch-up）
      3-6. 每根新柱:            六步 Alpha Radar 流水线
      7. 采集层节流日志        — backlog / drain 摘要

    v1.3.2 改动:
      - bar_asm.poll() 升级为 bar_asm.poll_with_drain()
        单轮主循环内允许连续 poll 直到追上市场或触发硬上限
      - SnapshotRecorder.write() 传入采集度量，raw_values 新增 2 字段
      - 新增 backlog 告警 + drain 摘要的节流日志（防刷屏）
    """
    ...
    log_info("Alpha Radar v1.3.2 启动。K={}, volume_bar_n={} BTC, "
    ...
    # v1.3.2: 节流日志计时器
    # 两个独立计时器防止"backlog 告警"和"drain 摘要"彼此覆盖
    ...
    while True:
        try:
            ...
            # ── 节流日志: backlog 告警 ─────────────────────────────
            # 只在 backlog 未消化时告警（drain 用完预算仍没追上），
            # 单纯 drain 轮次 > 1 不是告警——这恰恰说明 drain 在正常工作。
            now_sec = int(time.time())
            ...
            # ── 节流日志: drain 周期摘要 ───────────────────────────
            # 只有在 drain 真的发生（rounds > 1 或有显著耗时）时才打
            is_drain_noteworthy = (
            ...
            for bar in new_bars:
                ...
                # v1.3.2: 传入 collection_metrics 供 raw_values 记录采集度量
                # 注: 本轮 drain 产出的多根 Bar 共享同一组 cycle_metrics
                #     因为它们是"同一次 drain 周期"的产物，度量语义一致。
