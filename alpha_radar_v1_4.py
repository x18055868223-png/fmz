# -*- coding: utf-8 -*-
"""
Alpha Radar v1.4  (FMZ 单文件策略)
================================================================
研究型价格结构观测系统。无任何实盘发单逻辑。
蓝图: alpha_radar_spec v1.4 / 参考: gamma_spatial_observer_v6.py
...
v1.1 修复: reset_ppe_history / 吸收趋势冻结语义 / OLS 窗口 gap_closure 清空
v1.2 修复: 去趋势 σ_slow / 相对价格硬护栏 / Chart Flag 重构 / 事件持久化 / 事件流表
v1.3 新增: 带内结构健康观测层 (ClassificationEvidence 中心性因子 + anchor_health_score)
v1.3.1: 6 项外科式修补
v1.3.2: poll_with_drain + Display 三态语义 + LogProfit 改健康度出口
v1.3.3: 任务调度边界收口 (time-driven / event-driven) + 《数据采集》状态栏
v1.3.4: 锚健康度 readiness 语义收口 (去掉 runtime_gate 一票否决)

v1.4 运行审计层 (Runtime Audit):
  新增独立 DebugRecorder 模块, 不改业务数据流。职责:
    - 本地结构化 JSONL 落盘到 FMZ 工作目录
    - 覆盖主循环审计 / Bar 流水线 / 因子审计 / 任务频率 / 状态迁移 /
      异常 / 启动元信息 七大类
    - 失败降级: 连续写失败自动禁用持久化, 业务逻辑不受影响
    - 零副作用: debug_enabled=False 时所有 record_* 方法退化为空 no-op
    - 单向依赖: 业务代码只调用 debug_rec.record_*, 从不读 debug_rec

  目录: /home/bitnami/logs/storage/654434/alpha_radar_debug/
        按 run_id 隔离 (每次启动一个新子目录)
        latest_run.txt 指向最新 run_id 便于回传

  Unready Reason Code 体系:
    引入全局 reason code 字典, 所有 unready_reason 字段统一使用。
    让后续回放能直接判断"为什么某因子失效", 而不是只看到 None。
================================================================
"""
...
CONFIG = {
    ...
    # ── v1.4: 运行审计模块 (DebugRecorder) ───────────────────────
    # 纯观测层, 不影响业务逻辑。debug_enabled=False 时所有 record_*
    # 退化为 no-op, 零 I/O 零字典构造。
    "debug_enabled":                True,
    "debug_base_dir":               "/home/bitnami/logs/storage/654434/alpha_radar_debug",
    # 单个文件软上限, 超限后停写该文件 (不影响其他文件, 不影响业务)
    "debug_file_size_limit_mb":     100,
    # 主循环级 cycle_audit 记录节流: 每 N 轮记录 1 条
    # =1 时每轮都记 (最详细); 正常运行流量偏大时可设 5 或 10
    "debug_cycle_audit_every":      1,
    # 任务调度审计记录策略:
    #   "on_execute"  : 仅记录实际执行的任务 (默认, 日志量小)
    #   "on_attempt"  : 每次尝试调度都记录 (含 skipped), 日志量大但最完整
    "debug_task_audit_mode":        "on_execute",
    # 连续写失败阈值; 超过后该文件持久化禁用, 其他文件不受影响
    "debug_persist_failure_limit":  5,
}


# ================================================================
# v1.4: 全局 Reason Code 字典 (unready_reason / skipped_reason)
# ================================================================
# 所有 DebugRecorder 日志中的 reason 字段必须从这个字典取值, 保证
# 命名稳定, 便于后续回放解析。按"计算层"、"调度层"、"完整性"分类。
# 任何新增 reason 必须加入本字典, 不允许散落自由字符串。
REASON_CODE = {
    # ── 计算层 (因子未就绪) ──────────────────────────────────────
    "ANCHOR_SOURCE_MISSING":       "anchor_source_ts_ms 缺失或为 0 (GEX 从未成功)",
    "ANCHOR_EXPIRED":              "anchor_state == EXPIRED (锚参照系失效)",
    "CENTRALITY_BUFFER_COLD":      "中心性缓冲未满 centrality_min_bars",
    "CENTRALITY_FROZEN_NO_TRANSIT":"冻结期且无透出值 (首次冻结即发生)",
    "PPE_HISTORY_COLD":            "PPE 历史 < ppe_history_window (20K)",
    "OLS_WINDOW_COLD":             "OLS 窗口 < ols_window_min (3K)",
    "R_SQUARED_BELOW_MIN":         "R² < ols_r2_min",
    "CVD_DEGRADED":                "trade_id gap 导致 CVD 方向降级",
    "COLD_START_WARMUP":           "其他 warm-up (如首根 Bar / 无前值)",
    # ── 调度层 (任务跳过) ────────────────────────────────────────
    "INTERVAL_NOT_REACHED":        "距离上次执行未达 interval",
    "QUEUE_EMPTY":                 "pending 队列为空, 无数据可刷新 (tick_chart)",
    "SNAPSHOT_NONE":               "latest_snapshot 未就绪 (tick_summary)",
    "SCORE_NONE":                  "anchor_health_score=None, 跳过以避免伪造 (tick_logprofit)",
    "CHART_FN_MISSING":            "FMZ Chart 函数未注入, 无法刷新",
    "LOGPROFIT_FN_MISSING":        "FMZ LogProfit 函数未注入",
    "DRAIN_BELOW_NOTEWORTHY":      "drain 不值得记录 (rounds=1 且耗时<500ms)",
    "BACKLOG_NOT_TRIGGERED":       "未处于 backlog 状态",
    # ── 完整性 / 异常 ────────────────────────────────────────────
    "BACKLOG_ACTIVE":              "Backlog 未消化 (drain 用完预算仍未追上)",
    "BAND_CLAMPED":                "band_half 触发硬护栏截断",
    "GEX_FETCH_FAILED":            "GEX HTTP 失败或 payload 解析失败",
    "PERSIST_DISABLED":            "持久化因连续失败已禁用",
}
...
# ================================================================
# v1.4 SECTION: DebugRecorder (运行审计模块, 独立于业务)
# ================================================================

class DebugRecorder:
    """
    v1.4: 运行审计层。独立于业务模块, 只写不读, 单向依赖。

    设计约束:
      1. debug_enabled=False 时所有 record_* 方法立即 return, 零开销。
      2. 写失败只 WARN, 连续失败超过阈值则禁用该文件 (其他文件不受影响)。
      3. 业务代码只调用本类的 record_*, 从不读 DebugRecorder 状态。
      4. 每次启动生成独立 run_id 目录, 重启即隔离, 不做 rotation。

    职责分工:
      - run_meta.json        : 启动一次性写入
      - cycle_audit.jsonl    : 每轮主循环 (按 debug_cycle_audit_every 节流)
      - bar_pipeline.jsonl   : 每根完成 Bar
      - factor_audit.jsonl   : 每根 Bar 的因子计算与 unready 原因
      - task_frequency.jsonl : time-driven 任务的 attempted / executed / skipped
      - state_transitions.jsonl: 四轴 + event_state 任一发生迁移
      - anomalies.jsonl      : backlog / cvd_degraded / band_clamp 等结构性异常
      - exceptions.jsonl     : 模块异常 + traceback

    注: 本类的所有方法实现了"宽容失败"语义 —— 写盘失败不抛异常,
        只记录内部 _persist_failures 计数, 超过阈值则软禁用。
    """

    # 文件名常量 —— 统一字段, 避免魔法字符串散落
    F_META        = "run_meta.json"
    F_CYCLE       = "cycle_audit.jsonl"
    F_BAR         = "bar_pipeline.jsonl"
    F_FACTOR      = "factor_audit.jsonl"
    F_TASK        = "task_frequency.jsonl"
    F_TRANSITION  = "state_transitions.jsonl"
    F_ANOMALY     = "anomalies.jsonl"
    F_EXCEPTION   = "exceptions.jsonl"

    def __init__(self):
        self._enabled = bool(CONFIG.get("debug_enabled", False))
        self._base_dir = CONFIG.get("debug_base_dir", "") or ""
        self._run_id = ""
        self._run_dir = ""
        # 每个文件独立的失败计数 + 禁用标志, 文件之间隔离
        self._persist_failures = {}
        self._persist_disabled = {}
        # 文件字节数累计 (soft-cap 判断用, 避免反复 stat)
        self._file_bytes = {}
        self._size_limit_bytes = int(
            CONFIG.get("debug_file_size_limit_mb", 100) * 1024 * 1024)
        # 启动时间戳, 由 boot() 填充
        self._start_ts_ms = 0
        # 任务调度的上次执行时间记录 (为 task_frequency 提供 actual_interval)
        self._last_task_ts_ms = {}

    # ════════════════════════════════════════════════════════════
    # 启动与生命周期
    # ════════════════════════════════════════════════════════════

    def boot(self, config_summary):
        """
        启动时调用一次。生成 run_id 目录, 写 run_meta.json。
        失败只 WARN, 不抛异常 —— debug 禁用比业务挂掉重要。
        """
        if not self._enabled:
            return
        try:
            self._start_ts_ms = now_ms()
            # 生成 run_id: YYYYMMDD_HHMMSS_<short>
            # short 是基于 start_ts_ms 的简单哈希, 4 位十六进制, 同秒不冲突足够
            start_dt = (datetime.datetime.utcfromtimestamp(
                self._start_ts_ms / 1000.0) + datetime.timedelta(hours=8))
            short = format(self._start_ts_ms & 0xFFFF, "04x")
            self._run_id = start_dt.strftime("%Y%m%d_%H%M%S") + "_" + short
            self._run_dir = self._base_dir + "/run_" + self._run_id

            # 创建 base_dir 和 run_dir (os.makedirs 可能在 FMZ 沙盒受限)
            import os
            os.makedirs(self._run_dir, exist_ok=True)

            # 写 run_meta.json (单次, 直接覆盖)
            meta = {
                "run_id":          self._run_id,
                "start_ts_ms":     self._start_ts_ms,
                "utc8_datetime":   fmt_datetime_ms_utc8(self._start_ts_ms),
                "system_name":     "alpha_radar",
                "system_version":  "v1.4",
                "code_version":    "alpha_radar_v1_4.py",
                "debug_dir":       self._run_dir,
                "config_summary":  config_summary,
            }
            meta_path = self._run_dir + "/" + self.F_META
            with open(meta_path, "w", encoding="utf-8") as f:
                json.dump(meta, f, ensure_ascii=False, indent=2)

            # latest_run.txt 写 run_id, 便于回传
            latest_path = self._base_dir + "/latest_run.txt"
            try:
                with open(latest_path, "w", encoding="utf-8") as f:
                    f.write(self._run_id + "\n")
            except Exception:
                pass  # latest_run.txt 失败不致命

            log_info("DebugRecorder: 运行审计启用, run_id=" + self._run_id)
            log_info("DebugRecorder: 日志目录=" + self._run_dir)
        except Exception as error:
            log_warn("DebugRecorder: boot 失败, 禁用审计持久化 -- " + str(error))
            self._enabled = False
    ...
    def _append_jsonl(self, filename, record):
        """
        追加一行 JSON 到指定文件。
        宽容失败: 写盘异常不抛出, 只计入文件级失败计数。
        超过 debug_persist_failure_limit 次失败即软禁用该文件。
        超过 debug_file_size_limit_mb 即软禁用, 避免无限膨胀。
        """
        if not self._enabled:
            return
        if self._persist_disabled.get(filename, False):
            return
        ...
        try:
            ...
            # soft-cap 检查
            current_bytes = self._file_bytes.get(filename, 0)
            if current_bytes + line_bytes > self._size_limit_bytes:
                self._persist_disabled[filename] = True
                log_warn("DebugRecorder: " + filename + " 超 "
                         + str(CONFIG["debug_file_size_limit_mb"])
                         + "MB 软上限, 已禁用该文件")
                return
            ...
            with open(path, "a", encoding="utf-8") as f:
                f.write(line + "\n")
            self._file_bytes[filename] = current_bytes + line_bytes
            # 成功后重置失败计数
            if self._persist_failures.get(filename, 0) > 0:
                self._persist_failures[filename] = 0
        except Exception as error:
            fail_count = self._persist_failures.get(filename, 0) + 1
            self._persist_failures[filename] = fail_count
            log_warn("DebugRecorder: " + filename + " 写失败 ("
                     + str(fail_count) + "): " + str(error))
            if fail_count >= CONFIG.get("debug_persist_failure_limit", 5):
                self._persist_disabled[filename] = True
                log_warn("DebugRecorder: " + filename
                         + " 连续失败达阈值, 已禁用")

    def _base_envelope(self, kind, cycle_index=None, bar_index=None):
        """每条日志的公共信封: ts / run_id / kind / cycle/bar index"""
        ts = now_ms()
        env = {
            "ts_ms":         ts,
            "utc8_datetime": fmt_datetime_ms_utc8(ts),
            "run_id":        self._run_id,
            "kind":          kind,
        }
        if cycle_index is not None:
            env["cycle_index"] = cycle_index
    ...
    # ════════════════════════════════════════════════════════════
    # 记录接口 (业务代码的唯一入口)
    # 所有 record_* 在 debug_enabled=False 时零开销退出
    # ════════════════════════════════════════════════════════════

    def record_cycle_audit(self, cycle_index, cycle_metrics,
                           system_state, bar_index, new_bars_count,
                           tasks_summary, had_exception):
        """
        主循环级审计。每轮写 1 条 (按 debug_cycle_audit_every 节流)。

        tasks_summary: dict, key=task_name, value=dict({
            'executed': bool,
            'reason': REASON_CODE key or None  (不执行时说明)
        })
        """
        if not self._enabled:
            return
        every = int(CONFIG.get("debug_cycle_audit_every", 1))
        if every > 1 and (cycle_index % every) != 0:
            return

        rec = self._base_envelope("cycle_audit",
        ...
        rec.update({
            ...
            "trade_count":          cycle_metrics.get("trade_count"),
            "catchup_rounds":       cycle_metrics.get("catchup_rounds"),
            "wall_time_ms":         cycle_metrics.get("wall_time_ms"),
            "hit_limit":            cycle_metrics.get("hit_limit"),
            "hit_wall_time":        cycle_metrics.get("hit_wall_time"),
            "backlogged":           cycle_metrics.get("backlogged"),
            "tasks":                tasks_summary,
            "had_exception":        had_exception,
        })
        self._append_jsonl(self.F_CYCLE, rec)

    def record_bar_pipeline(self, cycle_index, bar, snapshot, dev_state,
                             raw_evidence, labels, system_state, instructions,
                             snapshot_written):
        """
        每根完成 Bar 的流水线主记录。
        snapshot 为 None 时, 使用 bar 和 raw_evidence 做 fallback。
        """
        if not self._enabled:
            return
        bar_index = bar.get("bar_index") if bar else None
        raw_row = snapshot.get("raw", {}) if snapshot else {}

        rec = self._base_envelope("bar_pipeline",
                                   cycle_index=cycle_index,
                                   bar_index=bar_index)
        rec.update({
            "bar_ts_ms":             bar.get("ts_ms") if bar else None,
            "bar_utc8":              fmt_datetime_ms_utc8(
                                         bar.get("ts_ms") if bar else None),
            ...
            "cvd_delta":             bar.get("cvd_delta") if bar else None,
            "flip_point":            raw_row.get("flip_point"),
            "band_half":             raw_row.get("band_half"),
            "band_clamped":          raw_row.get("band_clamped"),
            "normalized_deviation":  raw_row.get("normalized_deviation"),
            "event_state":           system_state.get("event_state"),
            "anchor_state":          system_state.get("anchor_state"),
            "runtime_gate":          system_state.get("runtime_gate"),
            "classification_state":  system_state.get("classification_state"),
            "outside_bar_count":     raw_row.get("outside_bar_count"),
            "anchor_shift_event":    raw_row.get("anchor_shift_event"),
            "shift_magnitude":       raw_row.get("shift_magnitude"),
            "instructions":          dict(instructions) if instructions else {},
            "raw_summary": {
                ...
                "centrality_buf":   raw_evidence.get("centrality_buffer_len"),
            },
            "labels_summary": {
        ...
        self._append_jsonl(self.F_BAR, rec)

    def record_factor_audit(self, cycle_index, bar_index, raw_evidence,
                             labels, system_state, unready_reasons):
        """
        因子计算审计。
        unready_reasons: dict, key=factor_name, value=REASON_CODE key
          (仅当该因子未就绪时才出现; 就绪因子不出现在 dict 中)
        """
        if not self._enabled:
            return
        rec = self._base_envelope("factor_audit",
                                   cycle_index=cycle_index,
                                   bar_index=bar_index)
        breakdown = labels.get("anchor_health_breakdown") or {}
        rec.update({
            "factors": {
                "ppe_raw":          raw_evidence.get("ppe_raw"),
                "ppe_percentile":   raw_evidence.get("ppe_percentile"),
            ...
            "health_breakdown": {
                "h_time":           breakdown.get("h_time"),
                "h_erosion":        breakdown.get("h_erosion"),
                "h_center_loss":    breakdown.get("h_center_loss"),
                "h_space":          breakdown.get("h_space"),
                "h_micro":          breakdown.get("h_micro"),
                "h_stability":      breakdown.get("h_stability"),
            },
            "anchor_health_score":   labels.get("anchor_health_score"),
            "anchor_health_level":   labels.get("anchor_health_level"),
            "centrality_frozen":     raw_evidence.get("centrality_frozen"),
            "unready_reasons":       unready_reasons,  # 逐字段 reason code
        })
        self._append_jsonl(self.F_FACTOR, rec)

    def record_task(self, cycle_index, task_name, expected_interval_sec,
                     executed, skipped_reason=None, extra=None):
        """
        任务调度审计。
        由 run() / Display.tick_* 在每次调度判定时调用。
        执行策略由 CONFIG["debug_task_audit_mode"] 控制:
          "on_execute" (默认): 只记录 executed=True 的
          "on_attempt"        : 每次尝试都记录 (含 skipped)
        """
        if not self._enabled:
            return
        mode = CONFIG.get("debug_task_audit_mode", "on_execute")
        if mode == "on_execute" and not executed:
            return

        ts = now_ms()
        last_ts = self._last_task_ts_ms.get(task_name, 0)
        since_last_ms = (ts - last_ts) if last_ts > 0 else None
        if executed:
            self._last_task_ts_ms[task_name] = ts

        actual_interval_sec = None
        if since_last_ms is not None:
            actual_interval_sec = since_last_ms / 1000.0

        lagged = False
        if executed and actual_interval_sec is not None and expected_interval_sec > 0:
            # 实际间隔超过预期的 2 倍视为 lag
            lagged = actual_interval_sec > (expected_interval_sec * 2.0)

        rec = self._base_envelope("task_frequency", cycle_index=cycle_index)
        rec.update({
            "task_name":             task_name,
            "expected_interval_sec": expected_interval_sec,
            "actual_interval_sec":   actual_interval_sec,
        ...
        if extra:
            rec["extra"] = extra
        self._append_jsonl(self.F_TASK, rec)

    def record_state_transition(self, cycle_index, bar_index,
                                 axis_name, from_state, to_state, cause=None):
        """四轴 + event_state 迁移时记录。稀疏事件。"""
        if not self._enabled:
            return
        if from_state == to_state:
            return  # 幂等: 未发生实际迁移不记录
        rec = self._base_envelope("state_transition",
                                   cycle_index=cycle_index,
                                   bar_index=bar_index)
        rec.update({
            "axis":       axis_name,
        ...
        self._append_jsonl(self.F_TRANSITION, rec)

    def record_anomaly(self, cycle_index, bar_index, anomaly_type,
                        detail=None):
        """
        结构性异常: backlog / cvd_degraded / band_clamp / gex_fetch_failed 等。
        anomaly_type 应使用 REASON_CODE 里的 key。
        """
        if not self._enabled:
            return
        rec = self._base_envelope("anomaly",
                                   cycle_index=cycle_index,
                                   bar_index=bar_index)
        rec.update({
            "anomaly_type": anomaly_type,
            "description":  REASON_CODE.get(anomaly_type, anomaly_type),
            "detail":       detail or {},
        })
        self._append_jsonl(self.F_ANOMALY, rec)

    def record_exception(self, module, stage, exception_obj,
                          cycle_index=None, bar_index=None, degraded=False,
                          affects_main_flow=False):
        """
        模块异常记录。traceback 被截断到 2KB 防止单条日志过大。
        """
        if not self._enabled:
            return
        tb = ""
        try:
            ...
            if len(tb) > 2048:
                tb = tb[:2048] + "...[truncated]"
        except Exception:
            tb = "(traceback unavailable)"

        rec = self._base_envelope("exception",
                                   cycle_index=cycle_index,
                                   bar_index=bar_index)
        rec.update({
            "module":             module,
            "stage":              stage,
            "exception_type":     type(exception_obj).__name__,
            "message":            str(exception_obj),
            "traceback":          tb,
            "degraded":           degraded,
            "affects_main_flow":  affects_main_flow,
        })
        self._append_jsonl(self.F_EXCEPTION, rec)

    # ════════════════════════════════════════════════════════════
    # 诊断辅助 (被业务模块自检使用)
    # ════════════════════════════════════════════════════════════

    def classify_health_unready_reasons(self, raw_evidence, system_state,
                                          anchor_source_ts_ms):
        """
        给定 raw_evidence 和 system_state, 返回健康度相关因子的 unready reason dict。
        仅记录为 None 的因子的 reason, 非 None 的不在 dict 中。
        这是 DebugRecorder 的诊断辅助, 不影响业务计算。
        """
        reasons = {}
        # anchor 级
        if not anchor_source_ts_ms:
            reasons["anchor_source_ts_ms"] = "ANCHOR_SOURCE_MISSING"
        if system_state.get("anchor_state") == "EXPIRED":
            reasons["anchor_state"] = "ANCHOR_EXPIRED"

        # 因子级
        buf_len = raw_evidence.get("centrality_buffer_len") or 0
        min_bars = CONFIG.get("centrality_min_bars", 0)
        cent_frozen = raw_evidence.get("centrality_frozen", False)
        if raw_evidence.get("ed1_raw") is None:
            if buf_len < min_bars:
                reasons["ed1_raw"] = "CENTRALITY_BUFFER_COLD"
            elif cent_frozen:
                reasons["ed1_raw"] = "CENTRALITY_FROZEN_NO_TRANSIT"
            else:
                reasons["ed1_raw"] = "COLD_START_WARMUP"
        if raw_evidence.get("center_loss") is None:
            if buf_len < min_bars:
                reasons["center_loss"] = "CENTRALITY_BUFFER_COLD"
            elif cent_frozen:
                reasons["center_loss"] = "CENTRALITY_FROZEN_NO_TRANSIT"
            else:
                reasons["center_loss"] = "COLD_START_WARMUP"
        if raw_evidence.get("erosion_drift") is None:
            if buf_len < min_bars:
                reasons["erosion_drift"] = "CENTRALITY_BUFFER_COLD"
            elif cent_frozen:
                reasons["erosion_drift"] = "CENTRALITY_FROZEN_NO_TRANSIT"
            else:
                reasons["erosion_drift"] = "COLD_START_WARMUP"
        if raw_evidence.get("ppe_percentile") is None:
            reasons["ppe_percentile"] = "PPE_HISTORY_COLD"
        if raw_evidence.get("cvd_degraded"):
            reasons["cvd_strength"] = "CVD_DEGRADED"
        return reasons
...
class Display:
    ...
    def tick_chart(self, anchor_ctx=None, bar_asm=None,
                    debug_rec=None, cycle_index=None):
        """
        v1.3.3: 固定时间任务 —— chart 线条刷新。
        v1.4: 可选 debug_rec 参数用于运行审计 (单向写入, 不影响任何行为)。
        ...
        """
        interval = CONFIG["chart_update_interval_sec"]
        now_sec = int(time.time())
        if now_sec - self._last_chart_update_sec < interval:
            if debug_rec:
                debug_rec.record_task(cycle_index, "tick_chart", interval,
                                       executed=False,
                                       skipped_reason="INTERVAL_NOT_REACHED")
            return
        ...
        if not chart_fn:
            ...
            if debug_rec:
                debug_rec.record_task(cycle_index, "tick_chart", interval,
                                       executed=False,
                                       skipped_reason="CHART_FN_MISSING")
            return
        ...
        try:
            ...
            if debug_rec:
                debug_rec.record_task(cycle_index, "tick_chart", interval,
                                       executed=True,
                                       extra={"bars_flushed": bars_flushed})
            ...
            self._pending_chart_queue = []
            if debug_rec:
                debug_rec.record_exception("Display", "tick_chart", error,
                                            cycle_index=cycle_index,
                                            degraded=True,
                                            affects_main_flow=False)
    ...
    def tick_status(self, anchor_ctx, bar_asm, snap_rec, cycle_index,
        ...
        "不伪造数据"如何兑现:
          ...
          - 采集时效性 / 数据采集 表的值来自 bar_asm / anchor_ctx
            ...
            不算伪造
        """
        interval = CONFIG["status_update_interval_sec"]
        now_sec = int(time.time())
        if now_sec - self._last_status_update_sec < interval:
            if debug_rec:
                debug_rec.record_task(cycle_index, "tick_status", interval,
                                       executed=False,
                                       skipped_reason="INTERVAL_NOT_REACHED")
            return
        ...
        try:
            ...
            log_warn("Display tick_status failed: " + str(error))
            if debug_rec:
                debug_rec.record_exception("Display", "tick_status", error,
                                            cycle_index=cycle_index,
                                            degraded=True,
                                            affects_main_flow=False)

    def tick_logprofit(self, debug_rec=None, cycle_index=None):
        """
        ...
        为什么 LogProfit 也转为 time-driven:
          ...
          断点是正确的视觉语义。
        """
        interval = CONFIG["logprofit_interval_sec"]
        now_sec = int(time.time())
        if now_sec - self._last_logprofit_sec < interval:
            if debug_rec:
                debug_rec.record_task(cycle_index, "tick_logprofit", interval,
                                       executed=False,
                                       skipped_reason="INTERVAL_NOT_REACHED")
            return
        ...
        if logprofit_fn:
            try:
                ...
                if debug_rec:
                    debug_rec.record_task(cycle_index, "tick_logprofit",
                                           interval, executed=True,
                                           extra={"score": round(float(score), 2)})
            except Exception as error:
                if debug_rec:
                    debug_rec.record_exception("Display", "tick_logprofit",
                                                error,
                                                cycle_index=cycle_index,
                                                degraded=True,
                                                affects_main_flow=False)
            ...
            if debug_rec:
                debug_rec.record_task(cycle_index, "tick_logprofit", interval,
                                       executed=False,
                                       skipped_reason="LOGPROFIT_FN_MISSING")

    def tick_summary(self, debug_rec=None, cycle_index=None):
        """
        v1.3.3: 固定时间任务 —— 综述日志。
        v1.4: 可选 debug_rec 参数用于审计。
        ...
        """
        interval = CONFIG["summary_log_interval_sec"]
        now_sec = int(time.time())
        if now_sec - self._last_summary_sec < interval:
            if debug_rec:
                debug_rec.record_task(cycle_index, "tick_summary", interval,
                                       executed=False,
                                       skipped_reason="INTERVAL_NOT_REACHED")
            return
...
def run():
    """
    v1.4 主策略循环 —— 任务调度器 + 运行审计层。

    每轮主循环 (loop_sleep_sec):
      [采集阶段]
        1. anchor_ctx.clear_cycle_flag()
      ...
      [time-driven 调度]
        display.tick_chart / tick_status / tick_logprofit / tick_summary
        backlog / drain 摘要日志 (run() 级别节流)

      [v1.4 新增 - 运行审计]
        - 每轮结束: debug_rec.record_cycle_audit (按节流)
        - 每根 Bar: debug_rec.record_bar_pipeline + record_factor_audit
        - 每次状态迁移: debug_rec.record_state_transition
        - 异常: debug_rec.record_exception
        审计层对业务零影响, debug_enabled=False 时所有 record_* 为 no-op。
    """
    ...
    # v1.4: 初始化运行审计。boot 失败不影响业务运行。
    debug_rec = DebugRecorder()
    debug_rec.boot(config_summary={
        "K":                       CONFIG["K"],
        "volume_bar_n":             CONFIG["volume_bar_n"],
        "loop_sleep_sec":           CONFIG["loop_sleep_sec"],
        "drain_enabled":            CONFIG["drain_enabled"],
        "max_drain_rounds":         CONFIG["max_drain_rounds"],
        "max_drain_wall_time_ms":   CONFIG["max_drain_wall_time_ms"],
        "chart_update_interval_sec":    CONFIG["chart_update_interval_sec"],
        "status_update_interval_sec":   CONFIG["status_update_interval_sec"],
        "logprofit_interval_sec":       CONFIG["logprofit_interval_sec"],
        "summary_log_interval_sec":     CONFIG["summary_log_interval_sec"],
        "gex_min_fetch_interval_ms":    CONFIG["gex_min_fetch_interval_ms"],
        "ppe_history_window":           CONFIG["ppe_history_window"],
        "centrality_min_bars":          CONFIG["centrality_min_bars"],
        "anchor_shift_frac":            CONFIG["anchor_shift_frac"],
        "anchor_ppe_reset_frac":        CONFIG["anchor_ppe_reset_frac"],
        "debug_enabled":                CONFIG["debug_enabled"],
        "debug_cycle_audit_every":      CONFIG["debug_cycle_audit_every"],
        "debug_task_audit_mode":        CONFIG["debug_task_audit_mode"],
    })

    log_info("Alpha Radar v1.4 启动。K={}, volume_bar_n={} BTC, "
    ...
    log_info("v1.4 调度: time-driven[chart={}s status={}s logprofit={}s summary={}s]".format(
    ...
    # v1.4: 状态迁移跟踪 —— 前一轮四轴快照, 用于 detect transition
    prev_axes = {
        "runtime_gate":         None,
        "anchor_state":         None,
        "event_state":          None,
    ...
    while True:
        ...
        try:
            ...
            if cycle_metrics["backlogged"]:
                ...
                # anomaly 每轮都记, 不节流 —— 回放时看到连续的 anomaly 行
                # 比稀疏的告警更能还原 backlog 持续程度
                debug_rec.record_anomaly(cycle_index, None, "BACKLOG_ACTIVE",
                    detail={
                        "drain_rounds": cycle_metrics["catchup_rounds"],
                        "wall_time_ms": cycle_metrics["wall_time_ms"],
                    })

            # CVD 降级也是结构性异常
            if bar_asm.is_cvd_degraded():
                debug_rec.record_anomaly(cycle_index, None, "CVD_DEGRADED",
                    detail={"note": "trade_id gap triggered"})
            ...
            # ══════════════════════════════════════════════════════
            for bar in new_bars:
                last_bar_index = bar.get("bar_index")
                ...
                try:
                    ...
                    debug_rec.record_exception("SnapshotRecorder", "write",
                        error, cycle_index=cycle_index,
                        bar_index=bar.get("bar_index"),
                        degraded=True, affects_main_flow=False)
                    raise

                # band_clamp 也是一个结构性异常
                if band_clamped:
                    debug_rec.record_anomaly(cycle_index, bar.get("bar_index"),
                        "BAND_CLAMPED", detail={"band_half": band_half})
                ...
                # Step G: Display.on_bar
                display.on_bar(
                    snapshot, anchor_ctx, bar_asm,
                    dev_state, labels, state_mgr.get_state(),
                    snap_rec)

                # ── v1.4 审计: 每根 Bar 的流水线 + 因子审计 ──────
                current_sys_state = state_mgr.get_state()
                debug_rec.record_bar_pipeline(
                    cycle_index, bar, snapshot, dev_state,
                    raw_evidence, labels, current_sys_state, instructions,
                    snapshot_written=snapshot_written)

                # factor_audit: 分析 unready 原因
                unready = debug_rec.classify_health_unready_reasons(
                    raw_evidence, current_sys_state,
                    anchor_ctx.get_source_ts_ms())
                debug_rec.record_factor_audit(
                    cycle_index, bar.get("bar_index"),
                    raw_evidence, labels, current_sys_state, unready)

                # v1.4 审计: 四轴 + event_state 迁移检测
                axes_now = {
                    "runtime_gate":         current_sys_state.get("runtime_gate"),
                    "anchor_state":         current_sys_state.get("anchor_state"),
                    "event_state":          current_sys_state.get("event_state"),
                    "classification_state": current_sys_state.get("classification_state"),
                }
                for axis_name, now_val in axes_now.items():
                    prev_val = prev_axes.get(axis_name)
                    if prev_val is not None and prev_val != now_val:
                        debug_rec.record_state_transition(
                            cycle_index, bar.get("bar_index"),
                            axis_name, prev_val, now_val,
                            cause="per_bar_observed")
                prev_axes = axes_now

            # ══════════════════════════════════════════════════════
            # time-driven 调度 (v1.3.3 核心, v1.4 透传 debug_rec)
            # ══════════════════════════════════════════════════════
            tasks_summary = {}

            # 记录任务执行前的挂钟, 便于判断 executed 状态
            before_chart = display._last_chart_update_sec
            display.tick_chart(anchor_ctx, bar_asm,
                                debug_rec=debug_rec, cycle_index=cycle_index)
            tasks_summary["tick_chart"] = {
                "executed": display._last_chart_update_sec != before_chart,
            }

            before_status = display._last_status_update_sec
            display.tick_status(anchor_ctx, bar_asm, snap_rec, cycle_index,
                                 debug_rec=debug_rec)
            tasks_summary["tick_status"] = {
                "executed": display._last_status_update_sec != before_status,
            }

            before_lp = display._last_logprofit_sec
            display.tick_logprofit(debug_rec=debug_rec, cycle_index=cycle_index)
            tasks_summary["tick_logprofit"] = {
                "executed": display._last_logprofit_sec != before_lp,
            }

            before_sum = display._last_summary_sec
            display.tick_summary(debug_rec=debug_rec, cycle_index=cycle_index)
            tasks_summary["tick_summary"] = {
                "executed": display._last_summary_sec != before_sum,
            }
            ...
            debug_rec.record_exception("run", "main_loop", error,
                                         cycle_index=cycle_index,
        ...
        # ══════════════════════════════════════════════════════
        # v1.4 审计: 主循环 cycle_audit (按 debug_cycle_audit_every 节流)
        # ══════════════════════════════════════════════════════
        try:
            current_sys_state = state_mgr.get_state()
        except Exception:
            current_sys_state = {}
        debug_rec.record_cycle_audit(
            cycle_index, cycle_metrics if 'cycle_metrics' in dir() else {},
            current_sys_state,
            last_bar_index,
            len(new_bars) if 'new_bars' in dir() else 0,
            tasks_summary if 'tasks_summary' in dir() else {},
            had_exception)
