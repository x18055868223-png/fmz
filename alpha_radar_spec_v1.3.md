...
v1.3 补上"带内结构健康"这一观测层。v1.2 能识别"破区"瞬间，但对破区前的"内部侵蚀"——做市商已失去"让价格回到中心"的能力但仍在维护边界——完全不可观测。v1.3 从"空间边界"向内延伸到"空间重心"，刻画带内结构的中心性退化。

**架构纪律**：不新增模块、不新增状态轴、不新增事件类型、不新增图表元素。健康度只做观测，不进入任何状态机门控。CVD 维持 v1.2 分类层角色不变。

### 13.1 带内中心性原始因子（ClassificationEvidence 新增）

**输入序列**：

```
_centrality_buffer: deque(maxlen=3K)
每根 Bar 的 d_capped = clamp(normalized_deviation, -d_cap, +d_cap)
d_cap = 1.5 (CONFIG["centrality_d_cap"])
```

截断目的：防止 CONFIRMED 阶段的极端偏离（+3σ 以上）污染带内健康模型。超过 1.5σ 的偏离其"侵蚀贡献"被饱和，剩余"脱离程度"信息由 OLS 和 event_state 负责。
...
|---|---|
| INSIDE | 更新（核心场景） |
| CANDIDATE | 更新（首次出带的 Bar 也是侵蚀证据） |
| REENTRY_PENDING | 更新（回带过程是中心性恢复的证据） |
| CONFIRMED | **冻结**（已脱离，中心性讨论失去物理意义） |

冻结期间所有原始值保留最后一次有效值，`centrality_frozen = True`。

**重置规则**：`reset_ppe_history` 指令（锚大位移时）同时清空 `_centrality_buffer` 和缓存（失去参照系）。`reset_ols_window` **不**清空（中心性与 OLS 窗口生命周期解耦）。

**冷启动**：`len(buffer) < K` 时全部输出 None，下游健康度因子对 None 取 0.5（中性）。

**三个正交原始因子**：

| 因子 | 公式 | 值域 | 物理语义 |
|---|---|---|---|
| ED1（指数加权重心） | `Σ(w_i × d_i) / Σ(w_i)`，半衰期 K | [-1.5, +1.5] | 近期重心偏移方向和幅度 |
| sign_consistency（加权方向一致性） | `|Σ(w_i × sign_eps(d_i))| / Σ(w_i)` | [0, 1] | 重心所在侧的时间加权确定性 |
| center_loss（加权 RMS 离心度） | `sqrt(Σ(w_i × d_i²) / Σ(w_i))` | [0, 1.5] | 锚作为"中心"的失效程度（方向无关） |

其中：
- `w_i = exp(-ln(2) × age_i / K)`，最新 Bar age=0
- `sign_eps(d)` 有 deadzone：`|d| < 0.10` → 0，否则 ±1。消除中心附近微小抖动的虚假方向信号

**三量独立性验证**（物理正交）：

| 场景 | ED1 | sign_consistency | center_loss | 含义 |
|---|---|---|---|---|
| 健康：小幅震荡 | ~0 | ~0 | ~0.2 | 锚中心性完好 |
| 单侧压迫 +0.7 | ~0.7 | ~1.0 | ~0.7 | 最危险的破区前兆 |
| 先 +1.2 后回 0 | ~0.2 | ~0.5 | ~0.5 | 锚验证了回吸能力 |
| 对称震荡 ±0.7 | ~0 | ~0 | ~0.7 | 锚中心性崩塌但非单侧 |

**衍生合成量**：

```
erosion_drift = |ED1| × sign_consistency    值域 [0, ~1.5]
```

物理语义：只在"重心偏移 AND 方向稳定"时为高值，对称震荡自动归零。供 H_space 使用。

**erosion_side 标签**（LabelGenerator）：
...
### 13.2 关于 sign_consistency 的修正说明

原方案使用等权计数法 `persistence_ratio = max(N_up, N_down) / N_total`。v1.3 实现时修正为加权方向一致性 `sign_consistency`（修正 A），理由：

1. 等权计数法对远期和近期 Bar 一视同仁，与 ED1 / center_loss 的指数加权时间语义不一致。
2. 计数法没有 deadzone，中心附近微小抖动（normalized_deviation = ±0.02）被视为有效方向信号，产生虚假的高 persistence。
3. 加权版本使用与 ED1 相同的指数权重和 deadzone，三个量的时间响应特性一致。

### 13.3 锚健康度评分（LabelGenerator 聚合）

**总分公式**：

```
anchor_health_score = 100 × H_time × H_space × H_micro × H_stability
```
...
| 方案 | 否决理由 |
|---|---|
| 扣分制 | step-function 跳变，维度权重隐式，[0,100] 边界溢出需 clamp |
| 线性加权平均 | 任一维度归零不能把总分拉到 0（GEX EXPIRED 时仍输出 50-60 分），无法表达"一票否决" |
| **乘法式因子链** | ✓ 采用。任一因子归零自动总分归零；多维度同时恶化时非线性惩罚；因子 ∈ [0,1] 乘积自然 ∈ [0,100] 无需 clamp |

**四个健康因子**：

**H_time — 分段时间常数**（消除 FRESH/STALE 切换跳变）：

```
age < T_fresh (180s):
    H_time = 1.0 - 0.1 × (age / T_fresh)           → 1.0 → 0.9 线性
T_fresh ≤ age < T_expired (3600s):
    τ = (T_expired - T_fresh) / 3
    H_time = 0.9 × exp(-(age - T_fresh) / τ)        → 0.9 → ~0.045
age ≥ T_expired:
    H_time = 0.0
```

两段交界 age = T_fresh 处两侧都是 0.9，连续平滑。

**H_space — 空间健康（几何平均，修正 B）**：

```
H_erosion = 1 / (1 + exp(8.0 × (erosion_drift - 0.6)))
H_center_loss = 1 / (1 + exp(8.0 × (center_loss - 0.8)))
H_space = sqrt(H_erosion × H_center_loss)
```

修正 B 说明：v1.3 是观测版不是门控版，直接乘积会因空间域双重扣分导致分数长期过低不可读。几何平均保持"两维度都差才严重扣分"的语义，但单维度差时更温和。

同时吃 erosion_drift 和 center_loss：只吃 erosion_drift 会漏掉对称震荡崩塌（erosion_drift=0 但 center_loss=0.7 的反例 B）。

**H_micro — PPE 百分位 sigmoid**：

```
H_micro = 1 / (1 + exp(6.0 × (ppe_percentile - 0.5)))
```

低百分位 = 强吸收 = 高健康。PPE 在此处用的是锚判定层的百分位（历史相对），不是分类层的 ppe_raw（绝对），因为健康度关心的是"与历史比当前吸收力如何"，这和锚判定层的视角一致。

**H_stability — RESETTING 期间线性爬升**：

```
RESETTING: H_stability = min(1.0, stable_bar_count / K)
其他状态:  H_stability = 1.0
```
...
| anchor_source_ts_ms = None | H_time = 0.5 |
| erosion_drift = None | H_erosion = 0.5 |
| center_loss = None | H_center_loss = 0.5 |
| ppe_percentile = None | H_micro = 0.5 |

中性值 0.5 的目的：不让单个缺值阻断全局打分，也不让缺值方向偏移总分。

**健康度等级分档**（仅 UI，不进入任何决策）：
...
| 60-80 | 良 | 轻微扣分 |
| 40-60 | 警 | 某维度明显恶化 |
| 20-40 | 危 | 多维度恶化或单维度严重恶化 |
| < 20 | 濒危 | 锚基本失效 |
| None | 未就绪 | 冷启动或关键数据缺失 |

### 13.4 CVD 的 v1.3 角色（显式声明）

**CVD 不进入 erosion_drift**：空间维度保持纯空间度量。CVD 是外部施加的成交压力，不是锚的内在属性。

**CVD 不进入 anchor_health_score**：避免双重计数（CVD 已在分类矩阵承担置信度维度）、避免把"被攻击"误等同于"不健康"（锚的价值在于"能顶住压力"）。

**CVD 维持 v1.2 角色不变**：分类矩阵的第二维（OLS × CVD），状态栏独立行展示。
...
| BarAssembler | 不动 |
| DeviationTracker | 不动 |
| ClassificationEvidence | 新增 `_centrality_buffer` + `_last_centrality_values` 缓存 + `_compute_centrality_factors()` 方法 + 6 个 raw 输出字段 |
| SystemStateManager | 仅补 `get_anchor_stable_bar_count()` getter |
| LabelGenerator | 签名扩展（+2 值参数），新增 `_make_erosion_side_label` + `_compute_anchor_health` + `_compute_h_time` + `_classify_health_level`，+4 个 label 输出字段 |
| SnapshotRecorder | raw_values 表 +6 字段，labels 表 +3 字段，不新增事件类型 |
| Display | 状态栏新增《带内结构健康评估》独立表（3 行），不新增图表系列 |

**不新增**：模块、状态轴、事件类型、图表系列、状态机门控。

### 13.6 CONFIG 新增项

```
# v1.3 中心性
centrality_window           = 3K    # 窗口与 OLS 对齐
centrality_min_bars          = K     # 冷启动阈值
centrality_ewma_halflife     = K     # 半衰期复用 K
centrality_d_cap             = 1.5   # normalized_deviation 截断
centrality_sign_eps          = 0.10  # 方向 deadzone
erosion_side_threshold       = 0.30  # |ED1| > 此值标注侵蚀方向

# v1.3 健康度
health_erosion_inflection    = 0.60  # H_erosion sigmoid 中心 [待校准]
health_erosion_beta          = 8.0   # H_erosion sigmoid 陡度 [待校准]
health_center_loss_inflection = 0.80 # H_center_loss sigmoid 中心 [待校准]
health_center_loss_beta      = 8.0   # [待校准]
health_micro_beta            = 6.0   # H_micro sigmoid 陡度 [待校准]
health_time_fresh_floor      = 0.90  # FRESH 期末端 H_time 下限
health_level_healthy         = 80    # UI 分档阈值
health_level_solid           = 60
health_level_stressed        = 40
health_level_critical        = 20
```

新增自由度 = 6（d_cap + 5 个 sigmoid/衰减参数），其余复用 K 或仅为 UI 分档。

### 13.7 反过拟合审查

**过拟合风险为零**的部分：所有新增参数只影响"观测数值"，不影响任何状态转换、事件触发、分类结论。校准不当只会让健康度曲线形状不理想，不会让系统决策出错。

**最容易被误用的地方**：anchor_health_score < 20 看起来像强烈信号，可能被拿来做"拒绝开仓"门槛。v1.3 没有验证过这个阈值的预测力。防御措施：规范和代码都标注"观测专用"，v1.4 决定是否升级前必须有至少 4 周实盘数据。

**显式延后到 v1.4+**：健康度图表曲线、健康度 threshold-crossing 事件、健康度进入 classification_state 门控、CVD 进入健康度、anchor_health_score 的斜率/加速度因子。

### 13.8 v1.3 raw_values 表新增字段

| 字段 | 类型 | 含义 |
|---|---|---|
| ed1_raw | float \| None | 指数加权重心（含符号） |
| sign_consistency | float \| None | 加权方向一致性 [0,1] |
| center_loss | float \| None | 加权 RMS 离心度 [0,1.5] |
| erosion_drift | float \| None | 合成侵蚀强度 |
| centrality_frozen | bool | CONFIRMED 期间冻结标记 |
| centrality_buffer_len | int | 中心性窗口当前长度 |

### 13.9 v1.3 labels 表新增字段

| 字段 | 类型 | 含义 |
|---|---|---|
| erosion_side | str | 上侵蚀 / 下侵蚀 / 中性 |
| anchor_health_score | float \| None | [0, 100] 连续评分 |
| anchor_health_level | str | 优 / 良 / 警 / 危 / 濒危 / 未就绪 |
...
**anchor_health_score 是未校准的观测指标**（v1.3 新增）：健康度评分的 sigmoid 参数（inflection、beta）均为经验初值，未经实盘验证。分档阈值（80/60/40/20）是 UI 便利性设定，不是物理推导结论。v1.3 显式禁止将健康度作为交易门槛或状态机输入使用，直到至少 4 周实盘数据验证其预测力。

**中心性因子在 CONFIRMED 期间冻结**（v1.3 新增）：带外期间"锚的中心性"不再有物理意义，冻结是正确的设计选择。但冻结期可能持续很长（如果脱离后长时间不回补），期间健康度的空间因子反映的是冻结前最后时刻的状态，不是实时状态。交易员应意识到这一点。

**center_loss 使用 RMS 而非 MAE**（v1.3 新增）：RMS 对极端值更敏感（弹簧势能物理模型），但也意味着单根异常 Bar 会对 center_loss 产生不成比例的影响。d_cap 截断提供了部分保护。若实盘发现过于敏感，v1.4 可切换为 MAE，仅改一行代码。
