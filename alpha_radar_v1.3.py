# -*- coding: utf-8 -*-
"""
Alpha Radar v1.3  (FMZ 单文件策略)
================================================================
研究型价格结构观测系统。无任何实盘发单逻辑。
蓝图: alpha_radar_spec v1.3 / 参考: gamma_spatial_observer_v6.py
...
class Display:
    ...
    def _maybe_add_event_flag(self, bar_index, bar_ts_ms,
        ...
        # 确定事件类型（优先级: departure > gap_closure > anchor_shift）
        event_type = None
        if dev_state.get("departure_confirmed_event"):
            event_type = "departure_confirmed"
        elif dev_state.get("gap_closure_event"):
            event_type = "gap_closure_confirmed"
        # 注: anchor_shift 事件由 raw_row.anchor_shift_event 追踪
        # 但此处 raw_row 不在本函数作用域内，改用 system_state 推断
        # 更稳健的做法是让 on_bar 传入 raw_row；但当前 raw_row
        # 已经通过 dev_state 无法访问 anchor_shift_event，
        # 因此这一分支在下面通过额外参数传入
        ...
        if event_type in ("departure_confirmed", "gap_closure_confirmed"):
            # 脱离确认时还没有分类结论，允许无条件打标
            # 回补确认时有分类，需要 HIGH/MEDIUM
    ...
    def _build_health_table(self, raw_row, labels, system_state):
        ...
        # 重算四因子用于分解展示 (不存入快照，只用于 UI)
        # 这里复用 LabelGenerator 的计算，但为了避免再造一个依赖，
        # 从 raw_safe + sys_safe 的已有字段推导近似分解。
        # 精确分解需要 anchor_source_ts_ms + stable_bar_count，
        # 状态栏这里退化为只展示总分，分解四因子由 LabelGenerator
        # 作为副产出保留在 labels 里。第一版简化: 展示不重算。
