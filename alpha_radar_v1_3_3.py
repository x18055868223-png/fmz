# -*- coding: utf-8 -*-
"""
Alpha Radar v1.3.3  (FMZ 单文件策略)
================================================================
研究型价格结构观测系统。无任何实盘发单逻辑。
蓝图: alpha_radar_spec v1.3.3 / 参考: gamma_spatial_observer_v6.py
...
v1.3.3 任务调度边界收口 + 数据采集状态栏增强:
  T-1 把任务明确分为 time-driven 与 event-driven:
      time-driven (由 run() 调度):
        - chart 线条刷新  (chart_update_interval_sec)
        - status 状态栏刷新 (status_update_interval_sec，新增)
        - summary_log      (summary_log_interval_sec)
        - logprofit        (logprofit_interval_sec)
        - backlog/drain 摘要日志
      event-driven (由 on_bar() 调度):
        - SnapshotRecorder.write
        - FSM / evidence / labels 流水线
        - chart flags (三类离散事件)
        - 事件日志 / 状态变化日志
  T-2 Display 引入 latest_* 缓存 + pending_chart_queue:
      on_bar() 只更新缓存和入队，不直接刷新输出
      run() 中定时任务消费缓存
  T-3 chart 线条刷新改为消费 pending queue，按真实 bar.ts_ms 顺序 add
      不伪造 wall-clock 点，不重复 add 同一点
  T-4 fmt_datetime_ms_utc8 新 helper，输出 YYYY-MM-DD HH:MM:SS
  T-5 状态栏《采集时效性》→《数据采集》重构为 5 行:
      (1) 成交量柱构造 (含未完成柱进度)
      (2) aggTrades 采集 (频率+本轮调用+累计+上次成功时间)
      (3) GEX 采集 (频率+本轮触发+累计+上次成功+freshness)
      (4) Backlog / 数据完整性
      (5) 运行轮次
  T-6 补最小必要 getters 和计数器:
      AnchorContext:  last_fetch_attempt_ms / last_fetch_success_ms /
                       fetch_attempt_count_total / fetch_success_count_total
      BarAssembler:   get_current_bar_build_progress /
                       get_last_success_fetch_ms /
                       get_poll_call_count_total
      run():          cycle_index 主循环轮次
================================================================
"""
...
class AnchorContext:
    ...
    # ── v1.3.3: 采集计数器 getter ────────────────────────────────
    def get_last_fetch_attempt_ms(self):
        """上次 HTTP 调用发起时刻 (不区分成功/失败)。0 表示从未发起。"""
        return self._last_fetch_attempt_ms
...
class Display:
    ...
    def _render_status(self, raw_row, labels, system_state,
                       anchor_ctx, bar_asm, snap_rec, cycle_index):
        """
        v1.3.3: 状态栏六表渲染主体 (原 _update_status)。
